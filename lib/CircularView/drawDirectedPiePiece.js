...
function drawDirectedPiePiece(_ref) {
  var _ref$tailThickness = _ref.tailThickness,
      tailThickness = _ref$tailThickness === undefined ? 0.6 : _ref$tailThickness,
  ...
  var path = (0, _path2.default)().moveto(arrowheadPoint.x, arrowheadPoint.y).lineto(arrowheadBottom.x, arrowheadBottom.y).lineto(arcLeftBottom.x, arcLeftBottom.y).arc({
    rx: tailInnerRadius,
    ry: tailInnerRadius,
    xrot: 0,
    ...
    x: arcRightBottom.x,
    y: arcRightBottom.y
  }).lineto(arcRightTop.x, arcRightTop.y).arc({
    rx: tailOuterRadius,
    ry: tailOuterRadius,
    xrot: 0,
    largeArcFlag: largeArcFlag,
    ...
    x: arcLeftTop.x,
    y: arcLeftTop.y
  }).lineto(arrowheadTop.x, arrowheadTop.y).closepath();
  path.print();
  return path;
}
