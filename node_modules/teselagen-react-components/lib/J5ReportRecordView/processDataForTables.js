...
var processJ5RunConstructs = function processJ5RunConstructs(j5RunConstructs) {
  return j5RunConstructs.filter(function (j5RunConstruct) {
    ...
    return _extends({}, j5RunConstruct, {
      ...
      nextLevelParts: ((0, _lodash.get)(j5RunConstruct, "sequence.sequenceParts") || []).map(function (part) {
        return part.name;
      }).join(", "),
      partsContainedNames: j5RunConstruct.partNames || (0, _lodash.get)(j5RunConstruct, "j5ConstructAssemblyPieces[0].assemblyPiece.j5AssemblyPieceParts[0].j5InputPart.sequencePart.name") && (0, _lodash.flatMap)(j5RunConstruct.j5ConstructAssemblyPieces, function (j5ConstructAssemblyPiece) {
...
var processJ5AnnealedOligo = function processJ5AnnealedOligo(j5Oligos) {
  return j5Oligos.filter(function (_ref3) {
    var oligo = _ref3.oligo;

    // only keep the j5 oligos that link to a top or bottom annealed oligo
    return oligo && oligo.j5AnnealedOligosTopOligos && oligo.j5AnnealedOligosBottomOligos && (oligo.j5AnnealedOligosTopOligos.length || oligo.j5AnnealedOligosBottomOligos.length);
  }).map(function (j5Oligo) {
    var targetPart = "";
    try {
      var topOligo = j5Oligo.oligo.j5AnnealedOligosTopOligos[0];
      var bottomOligo = j5Oligo.oligo.j5AnnealedOligosBottomOligos[0];
      var oligo = topOligo || bottomOligo;
...
exports.default = {
  ...
  j5InputPart: (0, _recompose.compose)(processInputParts, getInputPartsFromInputSequences),
