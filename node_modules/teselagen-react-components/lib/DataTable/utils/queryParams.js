...
exports.pageSizes = undefined;

var _typeof = typeof Symbol === "function" && typeof Symbol.iterator === "symbol" ? function (obj) { return typeof obj; } : function (obj) { return obj && typeof Symbol === "function" && obj.constructor === Symbol && obj !== Symbol.prototype ? "symbol" : typeof obj; };

var _extends = Object.assign || function (target) { for (var i = 1; i < arguments.length; i++) { var source = arguments[i]; for (var key in source) { if (Object.prototype.hasOwnProperty.call(source, key)) { target[key] = source[key]; } } } return target; };

exports.getMergedOpts = getMergedOpts;
exports.getCurrentParamsFromUrl = getCurrentParamsFromUrl;
exports.setCurrentParamsOnUrl = setCurrentParamsOnUrl;
exports.makeDataTableHandlers = makeDataTableHandlers;
...
var _tgClientQueryBuilder = require("tg-client-query-builder");

var _tgClientQueryBuilder2 = _interopRequireDefault(_tgClientQueryBuilder);
...
var additionalPageSize = window.frontendConfig && window.frontendConfig.additionalPageSize ? [(0, _lodash.toInteger)(window.frontendConfig.additionalPageSize)] : [];
var pageSizes = [5, 10, 15, 25, 50, 100, 200, 400].concat(additionalPageSize);

exports.pageSizes = pageSizes;
function getMergedOpts() {
...
function getFieldsMappedByCCDisplayName(schema) {
  return schema.fields.reduce(function (acc, field) {
    acc[(0, _lodash.camelCase)(field.displayName || field.path)] = field;
    return acc;
  }, {});
}
...
function orderEntitiesLocal(orderArray, entities, schema) {
  if (orderArray && orderArray.length) {
    ...
    orderArray.forEach(function (order) {
      ...
      if (path && (0, _lodash.endsWith)(path.toLowerCase(), "id")) {
        orderFuncs.push(function (o) {
          return parseInt((0, _lodash.get)(o, path), 10);
        });
      } else if (sortFn) {
        ...
        orderFuncs.push(function (r) {
          var val = (0, _lodash.get)(r, path);
          return val && val.toLowerCase ? val.toLowerCase() : val;
        });
      }
    });
    entities = (0, _lodash.orderBy)(entities, orderFuncs, ascOrDescArray);
  }
  return entities;
}
...
function getAndAndOrFilters(allFilters) {
  ...
  allFilters.forEach(function (filter) {
    if (filter.isOrFilter && typeof filter.filterValue === "string" && filter.filterValue.includes(",")) {
      // handle comma separated filters by adding more orWheres
      var allFilterValues = filter.filterValue.split(",");
      allFilterValues.forEach(function (filterValue, i) {
...
function filterEntitiesLocal() {
  ...
  if (allFilters.length) {
    ...
    var _getAndAndOrFilters = getAndAndOrFilters(allFilters),
        ...
        otherOrFilters = _getAndAndOrFilters.otherOrFilters;
    //filter ands first
    ...
    if (orFilters.length) {
      var orEntities = [];
      orFilters.concat.apply(orFilters, otherOrFilters).forEach(function (filter) {
        orEntities = orEntities.concat(getEntitiesForGivenFilter(entities, filter, ccFields));
      });
      entities = (0, _lodash.uniq)(orEntities);
    }
  }
  return entities;
}
...
function getEntitiesForGivenFilter(entities, filter, ccFields) {
  ...
  entities = entities.filter(function (entity) {
    var fieldVal = getValueToFilterOn ? getValueToFilterOn(entity) : (0, _lodash.get)(entity, path);
...
function getFiltersFromSearchTerm(searchTerm, schema) {
  ...
  if (searchTerm) {
    schema.fields.forEach(function (field) {
      ...
      var nameToUse = (0, _lodash.camelCase)(displayName || path);
...
selectedFilter, filterValue) {
  ...
  var stringFilterValue = filterValue && filterValue.toString ? filterValue.toString() : filterValue;
  stringFilterValue = stringFilterValue || "";
  var filterValLower = stringFilterValue.toLowerCase && stringFilterValue.toLowerCase();
  var arrayFilterValue = Array.isArray(filterValue) ? filterValue : stringFilterValue.split(".");
  if (ccSelectedFilter === "startsWith") {
    return qb ? qb.startsWith(stringFilterValue) //filter using qb (aka we're backend connected)
    : function (fieldVal) {
      ...
      return (0, _lodash.startsWith)(fieldVal.toLowerCase(), filterValLower);
    };
  } else if (ccSelectedFilter === "endsWith") {
    return qb ? qb.endsWith(stringFilterValue) //filter using qb (aka we're backend connected)
    : function (fieldVal) {
      //filter using plain old javascript (aka we've got a local table that isn't backend connected)
      if (!fieldVal || !fieldVal.toLowerCase) return false;
      return (0, _lodash.endsWith)(fieldVal.toLowerCase(), filterValLower);
    };
  } else if (ccSelectedFilter === "contains") {
    return qb ? qb.contains(stringFilterValue.replace(/_/g, "\\_")) //filter using qb (aka we're backend connected)
    : function (fieldVal) {
      //filter using plain old javascript (aka we've got a local table that isn't backend connected)
      if (!fieldVal || !fieldVal.toLowerCase) return false;
    ...
    return qb ? qb.inList(arrayFilterValue) //filter using qb (aka we're backend connected)
    : function (fieldVal) {
      //filter using plain old javascript (aka we've got a local table that isn't backend connected)
      if (!fieldVal || !fieldVal.toLowerCase) return false;
      return arrayFilterValue.map(function (val) {
    ...
    return qb ? qb.between(new Date(arrayFilterValue[0]), new Date(arrayFilterValue[1])) : function (fieldVal) {
      return (0, _moment2.default)(arrayFilterValue[0]).valueOf() <= (0, _moment2.default)(fieldVal).valueOf() && (0, _moment2.default)(fieldVal).valueOf() <= (0, _moment2.default)(arrayFilterValue[1]).valueOf();
    };
  } else if (ccSelectedFilter === "isBefore") {
    return qb ? qb.lessThan(new Date(filterValue)) : function (fieldVal) {
      return (0, _moment2.default)(fieldVal).valueOf() < (0, _moment2.default)(filterValue).valueOf();
    };
...
function getCurrentParamsFromUrl(location) {
  ...
  return parseFilters(_qs2.default.parse(search, { ignoreQueryPrefix: true }));
}
function setCurrentParamsOnUrl(newParams, replace) {
  var stringifiedFilters = stringifyFilters(newParams);
  replace({
    search: "?" + _qs2.default.stringify(stringifiedFilters)
  });
}
...
function stringifyFilters(newParams) {
  ...
  if (newParams.filters && newParams.filters.length) {
    filters = newParams.filters.reduce(function (acc, _ref, index) {
      ...
      acc += (index > 0 ? "::" : "") + (filterOn + "__" + (0, _lodash.camelCase)(selectedFilter) + "__" + safeStringify(Array.isArray(filterValue) ? filterValue.join(".") : filterValue));
      return acc;
    }, "");
  }
  var order = void 0;
...
function parseFilters(newParams) {
  return _extends({}, newParams, {
    ...
    filters: newParams.filters && newParams.filters.split("::").map(function (filter) {
      var splitFilter = filter.split("__");
      var filterOn = splitFilter[0],
...
function makeDataTableHandlers(_ref2) {
  ...
  function addFilters(newFilters, currentParams) {
    ...
    var filters = (0, _lodash.uniqBy)([].concat(newFilters, onlyOneFilter ? [] : currentParams.filters || []), "filterOn");
...
function getQueryParams(_ref3) {
  var currentParams = _ref3.currentParams,
      ...
      additionalOrFilter = _ref3.additionalOrFilter,
      doNotCoercePageSize = _ref3.doNotCoercePageSize,
      noOrderError = _ref3.noOrderError,
      isCodeModel = _ref3.isCodeModel;
  ...
  if (pageSize !== undefined && !doNotCoercePageSize) {
    ...
    var closest = (0, _lodash.clone)(pageSizes).sort(function (a, b) {
  ...
  if (isLocalCall) {
    ...
    if (!isInfinite) {
      ...
      newEntities = (0, _lodash.take)((0, _lodash.drop)(newEntities, offset), pageSize);
    }
    ...
    var qb = new _tgClientQueryBuilder2.default(model);
    // qb = qb.filter('user')
    // qb = qb.whereAny({
    ...
    if (tableQueryParams.order && tableQueryParams.order.length) {
      tableQueryParams.order.forEach(function (orderVal) {
        ...
        if (schemaForField) {
          ...
          !noOrderError && console.error("No schema for field found!", ccDisplayName, schema.fields);
        }
      });
    }
    ...
    var _getAndAndOrFilters2 = getAndAndOrFilters(allFilters),
        ...
        otherOrFilters = _getAndAndOrFilters2.otherOrFilters;

    var additionalFilterToUse = additionalFilter(qb, currentParams);
    var additionalOrFilterToUse = additionalOrFilter(qb, currentParams);
    try {
      var allOrFilters = [getQueries(orFilters, qb, ccFields)];
      otherOrFilters.forEach(function (orFilters) {
        allOrFilters.push(getQueries(orFilters, qb, ccFields));
      });
      allOrFilters.push(additionalOrFilterToUse);
      allOrFilters = allOrFilters.filter(function (obj) {
        return !(0, _lodash.isEmpty)(obj);
      });
      var allAndFilters = [getQueries(andFilters, qb, ccFields)];
      allAndFilters.push(additionalFilterToUse);
      allAndFilters = allAndFilters.filter(function (obj) {
        return !(0, _lodash.isEmpty)(obj);
      });
      if (allAndFilters.length) {
        qb.whereAll.apply(qb, allAndFilters);
      }
      if (allOrFilters.length) {
        qb.andWhereAny.apply(qb, allOrFilters);
      }
    } catch (e) {
      if (urlConnected) {
...
function getQueries(filters, qb, ccFields) {
  var subQueries = filters.reduce(function (acc, filter) {
    ...
    var filterValueToUse = filterValue;
    if (fieldSchema && fieldSchema.normalizeFilter) {
      filterValueToUse = fieldSchema.normalizeFilter(filterValue, selectedFilter, filterOn);
    }
    var subFilter = getSubFilter(qb, selectedFilter, filterValueToUse);
    if (fieldSchema) {
      var path = fieldSchema.path,
          reference = fieldSchema.reference;

      if (reference) {
        acc[reference.sourceField] = buildRef(qb, reference, (0, _lodash.last)(path.split(".")), subFilter);
      } else {
        acc[path] = subFilter;
      }
    } else if (filterOn === "id") {
