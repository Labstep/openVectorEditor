...
import { map, toString, camelCase, startCase, noop, isEqual, cloneDeep, keyBy, forEach } from "lodash";
import { Button, Menu, MenuItem, Classes, ContextMenu, Checkbox, Icon, Popover, Intent, Callout, Hotkey, Hotkeys, HotkeysTarget, Tooltip } from "@blueprintjs/core";
...
var DataTable = (_temp2 = _class = function (_React$Component) {
  ...
  DataTable.prototype.componentDidUpdate = function componentDidUpdate(oldProps) {
    ...
    if (tbody.scrollTop > 0) {
      ...
      var entitiesHaveChanged = oldEntities.length !== entities.length || getIdOrCodeOrIndex(entities[0] || {}) !== getIdOrCodeOrIndex(oldEntities[0] || {});
  ...
  DataTable.prototype.render = function render() {
    ...
    var propPresets = computePresets(this.props);
    ...
    if (selectedAndTotalMessage) {
      selectedAndTotalMessage = React.createElement(
        "div",
        null,
        selectedAndTotalMessage
      );
    ...
    return React.createElement(
      ...
      !noFooter && React.createElement(
        ...
        React.createElement(
          ...
          shouldShowPaging && React.createElement(PagingTool, computePresets(this.props))
        )
      )
    );
  };
  ...
  this.updateFromProps = function (oldProps, newProps) {
    ...
    setTimeout(function () {
      ...
      rowEl && tableBody && scrollIntoView(rowEl, tableBody, {
        alignWithTop: true
      });
    }, 0);
  };
  ...
  this.handleCopyHotkey = function () {
    ...
    _this3.handleCopySelectedRows(getRecordsFromIdMap(reduxFormSelectedEntityIdMap));
  };
  ...
  this.getRowCopyText = function (rowEl) {
    //takes in a row element
    if (!rowEl) return;
    return map(rowEl.children, function (cellEl) {
  ...
  this.handleCopySelectedRows = function (selectedRecords, e) {
    var _computePresets = computePresets(_this3.props),
    ...
    var textToCopy = map(rowEls, function (rowEl) {
  ...
  this.getTableRowProps = function (state, rowInfo) {
    ...
    return {
      ...
      onContextMenu: function onContextMenu(e) {
        ...
        var oldIdMap = cloneDeep(reduxFormSelectedEntityIdMap) || {};
        var newIdMap = void 0;
        if (withCheckboxes) {
          newIdMap = oldIdMap;
        } else {
          ...
          var keyedEntities = keyBy(entities, getIdOrCodeOrIndex);
          forEach(newIdMap, function (val, key) {
  ...
  this.renderCheckboxHeader = function () {
    ...
    var checkedRows = getSelectedRowsFromEntities(entities, reduxFormSelectedEntityIdMap);
    var checkboxProps = {
      checked: false,
      indeterminate: false
    };
    ...
    return !isSingleSelect ? React.createElement(Checkbox, _extends({
      ...
      , onChange: function onChange() {
        var newIdMap = cloneDeep(reduxFormSelectedEntityIdMap) || {};
  ...
  this.showContextMenu = function (idMap, e) {
    ...
    var selectedRecords = getRecordsFromIdMap(idMap);
    ...
    if (isCopyable && (selectedRecords.length === 0 || selectedRecords.length === 1)) {
      ...
      copyMenuItems.push(React.createElement(MenuItem, {
  ...
  this.renderColumnHeader = function (column) {
    ...
    var filterMenu = withFilter && !isActionColumn && !filterDisabled && !columnFilterDisabled ? React.createElement(
      BooleanValue,
      { defaultValue: false },
      function (_ref12) {
