...
import { pickBy, isNumber, startsWith, flatMap, isArray, take, flatten, isString } from "lodash";
import { Suggest } from "@blueprintjs/select";
import fuzzysearch from "fuzzysearch";
import "./style.css";
import { Popover, Position, Menu, Button, Hotkeys, HotkeysTarget, Hotkey } from "@blueprintjs/core";
import { createDynamicMenu, DynamicMenuItem } from "../utils/menuUtils";
import { comboToLabel } from "../utils/hotkeyUtils";

var MenuBar = HotkeysTarget(_class = (_temp2 = _class2 = function (_React$Component) {
  ...
  function MenuBar() {
    ...
    return _ret = (_temp = (_this = _possibleConstructorReturn(this, _React$Component.call.apply(_React$Component, [this].concat(args))), _this), _this.state = { searchVal: "", isOpen: false, openIndex: null }, _this.handleInteraction = function (index) {
          ...
          return {
            ...
            text: React.createElement(Suggest, _extends({
              ...
              inputProps: _extends({
                ...
                placeholder: "Search the menus (" + comboToLabel(_this.props.menuSearchHotkey || menuSearchHotkey, false).replace(/\s/g, "") + ")"
              }, inputProps),
              initialContent: null,
      ...
      return React.createElement(DynamicMenuItem, _extends({
        key: b.index
      }, {
        ...
        def: {
          ...
          label: i.path.length && React.createElement(
            "span",
            { style: { fontSize: 8 } },
            flatMap(i.path, function (el, i2) {
              if (i2 === 0) return el;
              return [" > ", el];
            })
          ),
  ...
  MenuBar.prototype.renderHotkeys = function renderHotkeys() {
    return React.createElement(
      Hotkeys,
      null,
      isNumber(this.menuSearchIndex) && React.createElement(Hotkey, {
        allowInInput: true,
        global: true,
  ...
  MenuBar.prototype.render = function render() {
    ...
    return React.createElement(
      ...
      this.addHelpItemIfNecessary(menu).map(function (topLevelItem, i) {
        var dataKeys = pickBy(topLevelItem, function (value, key) {
        ...
        return !topLevelItem.submenu ? button : React.createElement(
          Popover,
          {
            ...
            content: React.createElement(
              Menu,
              null,
              createDynamicMenu(_this2.addHelpItemIfNecessary(topLevelItem.submenu, i), enhancers)
            ),
...
var filterMenuItems = function filterMenuItems(searchVal, items) {
  var newItems = flatMap(items, function (item) {
    var text = item.text,
    ...
    if (fuzzysearch(searchVal.toLowerCase(), justText && justText.toLowerCase())) {
  ...
  return take(newItems, 10).map(function (i) {
    return _extends({}, i, {
...
function getStringFromReactComponent(comp) {
  ...
  if (!children) return "";
  if (isArray(children)) return flatMap(children, getStringFromReactComponent).join("");
...
function highlight(query, text, opts) {
  opts = opts || { tag: React.createElement("strong", null) };
